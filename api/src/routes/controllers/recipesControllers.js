...
const { API_KEY3, URL_RECIPES, URL_ID } = process.env;
...
const getApiRecipes = async function () {
  try {
    const infoApi = await axios.get(
      `${URL_RECIPES}${API_KEY3}&addRecipeInformation=true&number=50`
    );
...
const apiRecipeById = async function (id) {
  const response = await axios.get(`${URL_ID}${id}/information?${API_KEY3}`);
